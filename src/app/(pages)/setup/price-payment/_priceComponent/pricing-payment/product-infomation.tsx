...
const ProductInformation: React.FC<Props> = ({ onNext, onPrev }) => {
  ...
  const handleStartTrial = () => {
    // Logic to start the trial (e.g., API call or navigation)
    console.log("Free trial started");
    onNext(); // Move to the next step
  };

  const { data: currencyResponse, isLoading: isCurrency } =
    useGetAccountingListById({ id: "113" });
    ...
    <FormProvider {...methods}>
      ...
      <form action="" onSubmit={handleSubmit(onSubmit)}>
        <div className="lg:w-[80%] w-full">
          ...
          {showFreeTrial && (
            <div className="mt-4 p-4 border border-foundation-grey-grey-300 rounded-lg bg-foundation-white-white-400 relative">
              <button
                type="button"
                onClick={() => setShowFreeTrial(false)}
                className="absolute top-2 right-2 text-foundation-grey-grey-700 hover:text-foundation-grey-grey-900"
              >
                <MdCancel size={25} />{" "}
              </button>
              <h3 className="text-lg font-semibold text-foundation-black-black-400 mb-2">
                Free Trial
              </h3>
              <p className="text-sm text-foundation-grey-grey-700 mb-4">
                Enjoy a 30-day free trial of all modules with up to 3 users.
                Explore XYZ Business Management Applications and discover how
                they can enhance your operations.
              </p>
              <button
                type="button"
                onClick={handleStartTrial}
                className="w-full py-2 bg-foundation-purple-purple-400 text-white rounded-md hover:bg-foundation-purple-purple-300 transition-colors"
              >
                Start Free Trial
              </button>
            </div>
