"use client";
import {
  useCreateAccountingList,
  useUpdateAccountingList,
} from "../../hooks/mutate";
import { useGetAccountingListById } from "../../hooks/query";
...
const CreateList: React.FC = () => {
  ...
  const { data, error, isFetching } = useGetAccountingListById({
    id: accountId,
  });
  const lists = data || [];
  ...
  const validateFields = () => {
    ...
    const subCategoryNeeded =
      checkedState.subCategory &&
      !categories.some((category) => category.ListSubCategory?.length > 0);
  ...
  const addField = () => {
    setListFields([
      ...listFields,
      { label: "", value: "", dataType: "", visible: true },
    ]);
  };
  ...
  const handleInputChange = (
    ...
    setListFields(
      listFields?.map(
        (field, i) => (i === index ? { ...field, value: value } : field) // Update both label and value
      )
    );
  };
  ...
  const handleEditSubCategory = (categoryId: number, subCategoryId: number) => {
    ...
    const subCategory = category?.ListSubCategory?.find(
    ...
    setCategories((prevCategories) =>
      prevCategories.map((category) =>
        category.ID === categoryId
          ? {
              ...category,
              ListSubCategory: category.ListSubCategory.filter(
                (sub) => sub.ID !== subCategoryId
              ),
            }
          : category
  ...
  const handleSaveList = () => {
    ...
    const uniqueFields = listFields.filter(
      (field, index, self) =>
        field.value && self.findIndex((f) => f.value === field.value) === index
    );
    ...
    const mutationOptions = {
      ...
      onError: (error: any) => {
        setIsLoading(false);
        const errorMessage =
          error?.response?.data?.message || "Error saving list.";
  ...
  useEffect(() => {
    if (accountId && lists) {
      setListName((prevName) =>
        prevName !== lists?.data?.name ? lists?.data?.name : prevName
      );
      setListDescription((prevDescription) =>
        prevDescription !== lists?.data?.description
          ? lists?.data?.description
          : prevDescription
      );
      setCategories((prevCategories) =>
        JSON.stringify(prevCategories) !==
        JSON.stringify(lists?.data?.ListCategory)
          ? lists?.data?.ListCategory
          : prevCategories
      );

      setListFields((prevFields) => {
        const newFields =
          lists?.data?.column?.map((column: any) => ({
            label: column.name,
            value: column.name,
        ...
        return JSON.stringify(prevFields) !== JSON.stringify(newFields)
          ? newFields
          : prevFields;
      });
    }
        ...
        <div className="bg-secondary px-7 py-4 rounded-2xl text-[#434343]">
          ...
          <main className="w-full">
                    ...
                    <div className="relative w-full mb-4 md:mb-0">
                      ...
                      <CustomSelect
                        ...
                        handleDropdownChange={(option) =>
                          setSubCategory(option.listItem)
                        }
                    ...
                    {categories?.map((category) => (
                      ...
                      >
                        ...
                        {category?.ListSubCategory?.map((sub) => (
                          <div
                            key={sub.ID}
                            className="flex items-center justify-between pl-4 mt-3 text-gray-700 text-sm"
                          >
                            ...
                            <div className="flex gap-4 items-center">
                              {editSubCategoryId === sub.ID ? (
                                ...
                                <button
                                  onClick={() =>
                                    handleEditSubCategory(category.ID, sub.ID)
                                  }
                                  className="text-gray-600 flex items-center gap-1 hover:text-gray-800"
                                >
                                  <Pen size={18} /> Edit
            ...
            <div className="w-full">
              <table className="w-full">
                <thead className="w-full">
                  <tr className="text-[#374B58] text-[15px] font-[700]">
                    ...
                    <td className="py-5 gap-2 items-center">Data Type</td>
                    <td className="py-5 gap-2 items-center">Visibility</td>
                  </tr>
                </thead>
                <tbody>
              ...
              <div
                className="flex text-[#8133F1] items-center cursor-pointer gap-1 mb-5"
                onClick={addField}
              >
                <Plus /> Add a Field
              </div>
              <div className="flex gap-5">
                ...
                <button
                  className={`text-foundation-purple-purple-400 hover:bg-foundation-purple-purple-100 hover:text-white w-[32%] py-4 cursor-pointer border border-foundation-purple-purple-400 rounded-[16px]`}
                >
                  Cancel
                </button>
              </div>
