import { useUpdateChartOfAccount } from "@/app/(pages)/accounting/hooks/mutate";
import {
...
const AccountSummary: React.FC<AccountSummaryTabProps> = ({ proceed }) => {
  ...
  useEffect(() => {
    const grouped: GroupedAccounts = accounts.reduce(
      (acc: GroupedAccounts, account: Account) => {
  ...
  const filteredAndSearchedAccounts = Object.entries(groupedAccounts).reduce(
    (acc, [parentType, types]) => {
      acc[parentType] = Object.entries(types).reduce(
        (typeAcc, [type, accounts]) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const finalValues = {
        ...values,
        list_id: selectedAccountId
          ? Number(selectedLists[selectedAccountId]) || ""
          : "",
        isDefault: true,
      };
      ...
      <div className="w-[60%] pb-12">
        <div>
          {Object.keys(filteredAndSearchedAccounts).length > 0 ? (
            Object.entries(filteredAndSearchedAccounts).map(
              ([parentType, types], groupIdx) => (
                <div key={parentType}>
