...
import {
  useChartOfAccountByFilter,
  useGetAccountingListItemById,
  useGetParentAccountType,
...
const AccountDetails: React.FC<AccountDetailsTabProps> = ({
  proceed,
  formValues,
}) => {
  const [selectedParentType, setSelectedParentType] = useState<string | null>(
    formValues?.account_parent_type || null
  );
  const [selectedSubAccount, setSelectedSubAccount] = useState<string>(
    formValues?.sub_account_type || ""
  );
  const [filteredAccountType, setFilteredAccountType] = useState<any[]>([]);
  const [filteredAccountCode, setFilteredAccountCode] = useState<string | null>(
    null
  );
  const [isLoading, setIsLoading] = useState(false);

  const { data: parentAccountType } = useGetParentAccountType();
  const { data: chartOfAccount } = useChartOfAccountByFilter({
  ...
  useEffect(() => {
    const initialFilteredAccountType = selectedParentType
      ? parentAccountType?.find((item: any) => item.id === selectedParentType)
          ?.sub_account_types || []
      : [];

    const transformedAccountCodes = initialFilteredAccountType.map(
      (item: any) => ({
        ...item,
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (!id && !values.sub_account_type)
        errors.sub_account_type = "Account type is required";
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        const openingBalance =
          typeof values.opening_balance === "string"
            ? values.opening_balance.replace(/,/g, "")
            : values.opening_balance;

        const { account_parent_id, account_parent_type, ...otherValues } =
          values;

        const finalValues = {
          ...otherValues,
          ...(typeof account_parent_id === "number"
            ? { account_parent_id }
            : {}),
  ...
  return (
    <>
      <form onSubmit={formik.handleSubmit}>
        ...
        <p className="text-[14px] text-[#939292] mb-4">
          Kindly fill out your account information
        </p>
                ...
                ? LocationList?.data?.some((item: any) =>
                  ...
                  ? LocationList?.data
                      ?.map((item: any) => {
                        ...
                        return nameKey
                          ? { listItem: item[nameKey], value: item.id }
                          : null;
                      })
                      .filter(Boolean)
                  : ["No sublist found"]
        ...
        <div className="w-full md:w-[32%] mb-5">
          ...
          />
          {formik.errors.account_parent_type && (
            <div className="text-red-500 text-sm">
              {formik.errors.account_parent_type}
            </div>
          )}
        ...
        <div className="flex gap-4">
          <div className="w-full md:w-[32%] mb-5">
            ...
            <CustomSelect
              options={(Array.isArray(filteredAccountType)
                ? filteredAccountType
                : []
              ).map((acc: any) => ({
                listItem: acc.name,
            ...
            />
            {formik.errors.sub_account_type && (
              <div className="text-red-500 text-sm">
                {formik.errors.sub_account_type}
              </div>
            )}
        ...
        <div className="flex gap-4">
          ...
          <div className="w-full md:w-[32%] mb-5">
            ...
            />
            {formik.errors.account_parent_id && (
              <div className="text-red-500 text-sm">
                {formik.errors.account_parent_id}
              </div>
            )}
