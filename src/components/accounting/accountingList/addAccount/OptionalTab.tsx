...
} from "@/app/(pages)/accounting/hooks/mutate";
import { AccountDetailsFormValues } from "@/app/(pages)/accounting/accounting-list/add-account/page";
...
const OptionalTab: React.FC<OptionalTabProps> = ({ formValues }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      common_cost_allocation_type:
        formValues?.common_cost_allocation_type || "",
    ...
    validationSchema: Yup.object().shape({
      ...(checkedState.tax_line && {
        ...
        tax_description: Yup.string().required(
          "Tax Account Description is required"
        ),
      }),
      ...(checkedState.audit_line && {
        audit_sub_account: Yup.string().required(
          "Audit Sub-account is required"
        ),
        audit_account_name: Yup.string().required(
          "Audit Account Name is required"
        ),
        audit_account_code: Yup.string().required(
          "Audit Account Code is required"
        ),
        audit_description: Yup.string().required(
          "Audit Account Description is required"
        ),
      }),
      ...(checkedState.cost_line && {
        cost_type: Yup.string().required("Cost Type is required"),
        common_cost_allocation_type: Yup.string().required(
          "Common Cost Allocation type is required"
        ),
        cost_description: Yup.string().required(
          "Cost Account Description is required"
        ),
      }),
    }),
    onSubmit: (values) => {
    ...
    <>
      <form onSubmit={formik.handleSubmit}>
        <p className="text-xl font-medium mb-2 mt-2">
          Tax, Audit & Cost Line Mapping
        </p>
        <p className="text-[14px] text-[#939292] mb-5">
          You can map your tax, audit & cost line to the respective accounts.{" "}
        </p>
        <span className="mr-4">
          <CustomCheckbox
            label="Tax Line"
          ...
          <div className="w-full md:w-[32%] mb-5">
            ...
            <CustomSelect
              ...
              handleDropdownChange={(selectedOption) =>
                formik.setFieldValue("tax_type", selectedOption?.listItem)
              }
            ...
            {formik.errors.tax_type && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.tax_type as ReactNode}
              </div>
            )}
          </div>
          <div className="w-full md:w-[32%] mb-5">
            ...
            <CustomCombobox
              ...
              isDisabled={!checkedState.tax_line}
            />
            {formik.errors.tax_sub_account && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.tax_sub_account as ReactNode}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <div className="w-full md:w-[32%] mb-5">
            ...
            {formik.errors.tax_account_name && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.tax_account_name as ReactNode}
              </div>
            )}
          </div>
          ...
          <div className="w-full md:w-[32%] mb-5">
            ...
            {formik.errors.tax_code && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.tax_code as ReactNode}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="w-full md:w-[66%] mb-5">
          <label className="text-[15px] mb-2 block">
          ...
          <input
            ...
            disabled={!checkedState.tax_line}
          />
          {formik.errors.tax_description && (
            <div className="text-red-500 text-sm mt-2">
              {formik.errors.tax_description as ReactNode}
            </div>
          )}
        </div>
        ...
        <div className="w-full md:w-[32%] mb-5">
          ...
          <CustomSelect
            ...
            enableEffect={id ? true : false}
          />
          {formik.errors.audit_sub_account && (
            <div className="text-red-500 text-sm mt-2">
              {formik.errors.audit_sub_account as ReactNode}
            </div>
          )}
        </div>
        ...
        <div className="flex gap-4">
          <div className="w-full md:w-[32%] mb-5">
            ...
            />
            {formik.errors.audit_account_name && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.audit_account_name as ReactNode}
              </div>
            )}
          </div>
          ...
          <div className="w-full md:w-[32%] mb-5">
            ...
            <input
              ...
              disabled={!checkedState.audit_line}
            />
            {formik.errors.audit_account_code && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.audit_account_code as ReactNode}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="w-full md:w-[66%] mb-5">
          ...
          <input
            ...
            disabled={!checkedState.audit_line}
          />
          {formik.errors.audit_description && (
            <div className="text-red-500 text-sm mt-2">
              {formik.errors.audit_description as ReactNode}
            </div>
          )}
        </div>
        ...
        <div className="flex gap-4">
          <div className="w-full md:w-[32%] mb-5">
            ...
            <CustomCombobox
              ...
              handleDropdownChange={(selectedOption) =>
                formik.setFieldValue("cost_type", selectedOption?.listItem)
              }
              isDisabled={!checkedState.cost_line}
            />
            {formik.errors.cost_type && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.cost_type as ReactNode}
              </div>
            )}
          </div>
          <div className="w-full md:w-[32%] mb-5">
            ...
            <CustomCombobox
              ...
              isDisabled={!checkedState.cost_line}
            />
            {formik.errors.cost_sub_account && (
              <div className="text-red-500 text-sm mt-2">
                {formik.errors.cost_sub_account as ReactNode}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="w-full md:w-[32%] mb-5">
          ...
          <CustomCombobox
            ...
            isDisabled={!checkedState.cost_line}
          />
          {formik.errors.common_cost_allocation_type && (
            <div className="text-red-500 text-sm mt-2">
              {formik.errors.common_cost_allocation_type as ReactNode}
            </div>
          )}
        </div>
        ...
        <div className="w-full md:w-[66%] mb-5">
          ...
          {formik.errors.cost_description && (
            <div className="text-red-500 text-sm mt-2">
              {formik.errors.cost_description as ReactNode}
            </div>
          )}
        </div>
        ...
        <div className="mt-12">
          <button
            type="submit"
            className={
              "bg-foundation-purple-purple-400 border-none hover:bg-foundation-purple-purple-100 w-[32%] py-4 cursor-pointer text-foundation-white-white-400 rounded-[16px]"
            }
