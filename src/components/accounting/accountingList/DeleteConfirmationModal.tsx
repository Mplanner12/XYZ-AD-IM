import { deleteChartOfAccount } from "@/app/(pages)/accounting/api";
...
const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({
  ...
  const { refetch } = useGetChartOfAccount();
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      const response = await deleteChartOfAccount(accountID);
        ...
        <div className="bg-white p-6 rounded-2xl justify-center items-center shadow-md w-[18rem] md:w-[40rem] relative no-scrollbar">
          <div className="flex flex-col w-full md:w-[70%] justify-center pt-9 text-center m-auto mb-6">
            <MdDeleteOutline
              ...
              className="m-auto"
            />
            <p className="my-7 text-center text-[18px]">{TitleTopic}</p>
          </div>
