...
const ApprovedJournalTab: React.FC = () => {
  ...
  return (
    <>
        ...
        <main>
          <div className="flex justify-between my-3">
            <p className="text-xl font-medium">
              Approved Journal Entries ({journals?.length})
            </p>
            <div className="text-[15px]">
              ...
              <div className="text-[#8133F1] flex gap-2 justify-end">
                ...
                <button className="flex gap-1">
                  Comment <MessageSquareIcon size="16px" />
                </button>
              </div>
            </div>
          </div>
          <div className="flex justify-end text-[15px]"></div>
              ...
              <table className="min-w-[280%] md:min-w-[128%] lg:min-w-full text-[14px]">
                <thead className="w-full">
                  <tr className="text-[#374B58] text-[12px] border-[#eaecf0]  border-b  font-[500] bg-[#F5F5F5]">
                    ...
                    <td className="py-5 gap-2 items-center">Entry ID</td>
                    <td className="py-5 gap-2 items-center">Journal Type</td>
                    <td className="py-5 gap-2 items-center">Posted By</td>
                    <td className="py-5 gap-2 items-center">Status</td>
                    <td className="py-5 gap-2 items-center">Related Party</td>
                    <td className="py-5 gap-2 items-center">In-transfer</td>
                ...
                <tbody className="w-full bg-white">
                  {journals.length === 0 ? (
                    ...
                    (Array.isArray(journals) ? journals : []).map(
                      (journal: any, index: any) => (
                        ...
                        >
                          ...
                          <td className="py-6 text-left">{journal?.number}</td>
                          <td className="py-6 text-left">{journal?.type}</td>
                          ...
                          <td className="py-6 pr-5">
                            <CustomCheckbox
                              label=""
                              checked={journal?.entries[0]?.related_party}
                            />
                          </td>
                          <td className="py-6 pr-5">
                            <CustomCheckbox
                              label=""
                              checked={journal?.entries[0]?.in_transfer}
                            />
                          </td>
                          <td className="py-6 pr-5">
                            <TableDropDown
                              options={generateTableOptions(journal?.ID)}
                            />
                          </td>
                        </tr>
                      )
      ...
      <div>
        {selectedTab === "Journal-details" && (
          <ApprovedJournalDetails
            id={selectedId}
