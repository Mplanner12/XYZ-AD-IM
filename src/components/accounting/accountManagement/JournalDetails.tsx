...
} from "@/app/(pages)/accounting/api";
import {
  useGetAccountingListItemById,
  useGetGeneralJournalById,
...
const JournalDetails: React.FC<JournalDetailsProps> = ({
  ...
  const [selectedAccountingListId, setSelectedAccountingListId] = useState<
    string | undefined
  >();
  ...
  const { data: subListItem } = useGetAccountingListItemById({
    id: selectedAccountingListId,
  });
  const { data, refetch, isFetching } = useGetGeneralJournalById({
    id: journalId || "",
  });
  ...
  const handleOpenDeleteModal = (id: string) => {
    openModal(
      <DeleteModal
        id={id}
        text={"Are you sure you want to delete this entries?"}
      />
    );
  };
  ...
  const handleAccountSelection = (
    ...
    if (
      ...
      console.error(
        `Invalid journalIndex ${journalIndex} or entryIndex ${entryIndex}`
      );
      return;
    }
    ...
    <>
      <main className="pb-10">
        <div className="flex justify-between my-3">
          <p
            className="text-xl font-medium cursor-pointer flex"
            onClick={() => handleTabChange("Pending-journal")}
          >
            <ChevronLeft color="#8133F1" />{" "}
            {journalId ? "Journal Entry Details" : "All Journal Entry Details "}
          </p>
        </div>
            ...
            <div className="mb-3">
              ...
              <div className="flex gap-2 mb-2">
                <p className="text-[15px]">
                  Journal Type:{" "}
                  <span className="font-medium">{journalDetails?.type}</span>{" "}
                </p>
                <p className="text-[15px] ml-4">
                  Journal No:{" "}
                  <span className="font-medium">{journalDetails?.number}</span>
                </p>
              </div>
              <div className="flex gap-2 mb-2">
                <p className="text-[15px]">
        ...
        <div
          className={`flex ${
            journalId ? "justify-between " : "justify-end"
          } flex-wrap item-center w-[100%] mb-2`}
        >
          {journalId ? (
            <div className="flex">
              <p className="text-[15px]">
                Status:{" "}
                <span className="font-medium">{journalDetails?.status}</span>{" "}
              </p>
              <p className="text-[15px] ml-4">
                Approved By:{" "}
                <span className="font-medium mr-4">Segun adenuga</span>
                <span className="font-medium text-[#939292] ">
                  06-22-2022: 11:39 AM
                </span>
              </p>
            </div>
            ...
            <table className="min-w-[280%] md:min-w-[128%] lg:min-w-[140%] text-[14px]">
              <thead className="w-full">
                <tr className="text-[#374B58] text-[12px] border-[#eaecf0]  border-b  font-[500] bg-[#F5F5F5]">
                  ...
                  <td className="py-5 gap-2 items-center">Account Code</td>
                  <td className="py-5 gap-2 items-center">Account Name</td>
                  <td className="py-5 gap-2 items-center">Description</td>
                  <td className="py-5 gap-2 items-center">Customer Name</td>
                  <td className="py-5 gap-2 items-center">Debit</td>
                  <td className="py-5 gap-2 items-center">Credit</td>
                  <td className="py-5 w-[10%] pr-5">Sublist</td>
                  <td className="py-5 gap-2 pr-5 items-center">Action</td>
                </tr>
              </thead>
                      ...
                      return (
                        ...
                        >
                          {index === 0 && (
                            <td
                              className="py-6 pl-5"
                              rowSpan={journalDetails.entries.length}
                            >
                              <CustomCheckbox
                                checked={
                                  selectedCheckboxes[`entry-${index}`] || false
                                }
                                onChange={() =>
                                  handleCheckboxChange(`entry-${index}`)
                                }
                              />
                            </td>
                          )}
                                ...
                                <td className="py-6 flex gap-2 items-center">
                                  {isEditing ? (
                                    <DatePicker
                                      ...
                                      onChange={(date: any) =>
                                        handleDateChange(date, journalIndex)
                                      }
                                    />
                                  ) : (
                                  ...
                                  {isEditing ? (
                                    <div>
                                      <SelectDropDown
                                        ...
                                        menuWidth={9}
                                        label={
                                          journals?.entries?.associatedList
                                            ?.length
                                            ? ""
                                            : ""
                                        }
        ...
        <div className="flex gap-5">
          ...
          <button
            ...
            className={`text-foundation-purple-purple-400 hover:bg-foundation-purple-purple-100 hover:text-white w-[32%] py-3 cursor-pointer border boorder-foundation-purple-purple-400 rounded-[16px]`}
          >
            Revert Changes
          </button>
        </div>
