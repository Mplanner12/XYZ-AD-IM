...
const BankManagement = () => {
  ...
  const salesLocationsId = allLists?.find(
    (list: any) => list?.name === ListNames?.salesLocations
  )?.ID;
  const cashAndBankId = allLists?.find(
    (list: any) => list?.name === ListNames?.cashAndBank
  )?.ID;
  const employeeListId = allLists?.find(
    (list: any) => list?.name === ListNames?.employee
  )?.ID;
  const { data: locationOptions } = useGetAccountingListItemById({
    id: salesLocationsId,
  });
  const { data: cashBankOptions } = useGetAccountingListItemById({
    id: cashAndBankId,
  });
  const { data: employeeList } = useGetAccountingListItemById({
    id: employeeListId,
  });
