          ...
          <div className="flex items-center text-[16px]">
            {breadcrumbs.map((crumb, index) => (
              <React.Fragment key={index}>
                {index === 0 ? (
                  <Link href="/admin-setup/admin-dashboard" className="cursor-pointer">
                    {crumb}
                  </Link>
                ) : (
                  <button className="cursor-pointer" onClick={()=> router.push(`${moduleLink}`)}
                  >{crumb}</button>
                )}
          ...
          <ul className="divide-y divide-[#939292]">
            {[
              ...
              <li key={index} className="px-6 py-3 flex justify-between items-center">
                ...
                <Link href="#" className="text-purple-600 font-semibold text-sm">View</Link>
              </li>
            ))}
          </ul>
        </div>
